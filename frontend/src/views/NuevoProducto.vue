<template>
  <div class="max-w-2xl mx-auto p-6 bg-white rounded shadow">
    ...
    <form @submit.prevent="crearProducto" class="space-y-4">
      ...
      <!-- Categoría -->
      <CategorySelector v-model="producto.categorias" label="Categorías" />
...
const toFile = (x) => {
  ...
  if (FileCtor && x?.file instanceof FileCtor) return x.file
  if (FileCtor && x?.raw  instanceof FileCtor) return x.raw
  return null
}
...
const crearAtributo = async ({ nombre, valor }) => {
  try {
    ...
    const res = await api.post('atributos/', { atributo_id: base.id, valor })
...
onMounted(async () => {
  const [marcaRes, valoresRes, basesRes] = await Promise.all([
    api.get('marcas/'),
    api.get('atributos/'),
...
const crearProducto = async () => {
  ...
  try {
    const { data } = await postMultipart('productos/', formData)
