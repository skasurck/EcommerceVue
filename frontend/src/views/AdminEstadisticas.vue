<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <div class="flex items-center gap-1.5">
          <input type="date" v-model="desde" class="text-sm border border-slate-200 rounded-lg px-2 py-1.5 text-slate-700" />
          <span class="text-slate-400 text-sm">—</span>
          <input type="date" v-model="hasta" class="text-sm border border-slate-200 rounded-lg px-2 py-1.5 text-slate-700" />
          <button @click="cargar" class="px-3 py-1.5 bg-cyan-600 text-white text-sm rounded-lg hover:bg-cyan-700">
            Aplicar
          </button>
        </div>
      </div>
    ...
    <!-- Loading -->
    <div v-if="cargando" class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
    ...
    <template v-else-if="datos">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        >
          ...
          <p class="text-xl font-bold text-slate-800">{{ m.valor }}</p>
          <div v-if="datos.vs_anterior[m.key] !== null" class="mt-1.5 flex items-center gap-1">
            <span
            ...
            >
              <span>{{ datos.vs_anterior[m.key] >= 0 ? '▲' : '▼' }}</span>
              <span>{{ Math.abs(datos.vs_anterior[m.key]) }}%</span>
            </span>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          ...
          <Line :data="chartIngresos" :options="chartOpts('$')" class="max-h-64" />
        </div>

        <!-- Pedidos -->
        <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <h2 class="text-sm font-semibold text-slate-700 mb-4">Pedidos</h2>
          <Line :data="chartPedidos" :options="chartOpts('')" class="max-h-64" />
        </div>
      </div>

      <!-- Tablas: Top Productos y Categorías -->
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <!-- Top Productos -->
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div class="px-5 py-4 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Principales productos — Artículos vendidos</h2>
          </div>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-slate-400 uppercase border-b border-slate-100">
                ...
                <th class="px-5 py-2 text-right font-medium">Artículos</th>
                <th class="px-5 py-2 text-right font-medium">Ventas netas</th>
              </tr>
            </thead>
              ...
              <tr v-for="p in datos.top_productos" :key="p.slug" class="hover:bg-slate-50">
                <td class="px-5 py-3">
                  <router-link
                    ...
                    class="text-cyan-700 hover:underline font-medium line-clamp-1"
                  >
                    {{ p.nombre }}
                  </router-link>
                  <span v-else class="text-slate-700 line-clamp-1">{{ p.nombre }}</span>
                </td>
                <td class="px-5 py-3 text-right text-slate-700">{{ p.articulos }}</td>
                <td class="px-5 py-3 text-right text-slate-700">{{ fmt(p.ventas_netas) }}</td>
              </tr>
              <tr v-if="!datos.top_productos.length">
                <td colspan="3" class="px-5 py-6 text-center text-slate-400 text-sm">Sin datos en este período</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div class="px-5 py-4 border-b border-slate-100">
            <h2 class="text-sm font-semibold text-slate-700">Principales categorías — Artículos vendidos</h2>
          </div>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-slate-400 uppercase border-b border-slate-100">
                <th class="px-5 py-2 text-left font-medium">Categoría</th>
                <th class="px-5 py-2 text-right font-medium">Artículos</th>
                <th class="px-5 py-2 text-right font-medium">Ventas netas</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
              <tr v-for="c in datos.top_categorias" :key="c.nombre" class="hover:bg-slate-50">
                <td class="px-5 py-3 font-medium text-slate-700 uppercase text-xs tracking-wide">{{ c.nombre }}</td>
                <td class="px-5 py-3 text-right text-slate-700">{{ c.articulos }}</td>
                <td class="px-5 py-3 text-right text-slate-700">{{ fmt(c.ventas_netas) }}</td>
              </tr>
    ...
    <!-- Error -->
    <div v-else-if="error" class="bg-red-50 text-red-700 rounded-xl p-6 text-sm">
