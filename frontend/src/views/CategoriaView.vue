<template>
  <div class="min-h-screen bg-slate-50 dark:bg-[#060c18] transition-colors duration-200">

    <!-- ─── HERO HEADER ────────────────────────────────────────────── -->
    <div class="relative overflow-hidden bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-900 dark:to-[#060c18] border-b border-slate-200 dark:border-slate-800 pb-0 pt-10">
      ...
      <div v-if="currentCategory" class="pointer-events-none absolute inset-0 opacity-0 dark:opacity-100 transition-opacity duration-500"
           :style="`background: radial-gradient(ellipse 60% 60% at 20% 50%, ${getConfig(currentCategory.nombre).color}15, transparent)`"></div>

      <div class="relative mx-auto max-w-7xl px-4">
        <!-- Breadcrumb -->
        <nav class="mb-4 flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500">
          <RouterLink to="/" class="hover:text-slate-600 dark:hover:text-slate-300 transition-colors">Inicio</RouterLink>
          <svg class="h-3 w-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
          <RouterLink to="/categorias" class="hover:text-slate-600 dark:hover:text-slate-300 transition-colors">Categorías</RouterLink>
          <svg class="h-3 w-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
          <span class="text-slate-700 dark:text-slate-300 font-medium truncate max-w-[160px]">{{ currentCategory?.nombre ?? '…' }}</span>
        </nav>

        <!-- Title row -->
        <div class="flex items-center gap-4 pb-7">
          <div v-if="currentCategory"
               class="hidden sm:flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl transition-transform"
               :style="{ background: `linear-gradient(135deg, ${getConfig(currentCategory.nombre).color}22, ${getConfig(currentCategory.nombre).color}08)`,
                         border: `1px solid ${getConfig(currentCategory.nombre).color}35` }">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {{ totalProductos !== null ? `${totalProductos} producto${totalProductos !== 1 ? 's' : ''}` : 'Explorando…' }}
            </p>
          </div>
        </div>
      </div>
        ...
        <div ref="pillsScroll"
          ...
          <button
            ...
            :class="selectedSubId === null
              ? 'border-cyan-500 bg-cyan-500 text-white shadow-sm dark:bg-cyan-500/15 dark:text-cyan-300 dark:border-cyan-500'
    ...
    <div class="mx-auto max-w-7xl px-4 py-8">
      ...
      <!-- Empty -->
      <div v-else-if="productos.length === 0" class="py-24 text-center">
        ...
        <button v-if="selectedSubId" @click="selectSub(null)" class="mt-3 text-sm text-cyan-600 dark:text-cyan-400 hover:underline">
      ...
      <div v-if="loadingMore" class="flex justify-center py-8">
        <div class="h-8 w-8 rounded-full border-2 border-slate-200 dark:border-slate-700 border-t-cyan-500 animate-spin"></div>
      </div>
...
const CONFIGS = [
  ...
  { keys: ['impresión', 'impresion', 'copiado', 'toner', 'tinta'],  color: '#10b981', iconPath: '<path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0021 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 00-1.913-.247M6.34 18H5.25A2.25 2.25 0 013 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.056 48.056 0 011.913-.247m10.5 0a48.536 48.536 0 00-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5zm-3 0h.008v.008H15V10.5z"/>' },
