    ...
    <div class="mx-auto max-w-7xl">
      ...
      <div class="lg:hidden">
        ...
        <div class="h-12 flex items-center px-3 gap-2">
          ...
          <RouterLink to="/" class="flex items-center shrink-0">
            <img src="/logo-mktska.png" alt="Mktska Digital" class="h-8 w-auto object-contain [filter:brightness(0)] dark:[filter:brightness(0)_invert(1)] transition-[filter]" />
          </RouterLink>
        ...
        <!-- Fila 3: Ubicación de entrega -->
        <div class="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700/50 flex items-center gap-2 text-xs">
          ...
          <button type="button" @click="changeLocation" class="font-semibold text-slate-700 dark:text-slate-200 truncate hover:text-cyan-600 dark:hover:text-cyan-300 text-left">{{ locationLabel }}</button>
          <button type="button" @click="detectLocationByIP" class="ml-auto shrink-0 rounded bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 px-2 py-0.5 text-[10px] font-semibold text-slate-600 dark:text-slate-300">Auto</button>
        </div>
      </div>
